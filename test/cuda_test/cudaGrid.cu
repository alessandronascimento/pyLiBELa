...
__global__
void compute_grid_softcore_HB_CUDA(int npointsx, int npointsy, int npointsz,
                                  double grid_spacing,
...
__global__
void compute_grid_hardcore_HB_CUDA(int npointsx, int npointsy, int npointsz,
                                  double grid_spacing,
                                  double xbegin, double ybegin, double zbegin,
    ...
    if (i < npointsx && j < npointsy && k < npointsz){
        ...
        for (int a = 0; a < N; a++){
            ...
            solv += ((solvation_alpha * charges[a] * charges[a])+ solvation_beta) *  exp((-d2/(2*sigma*sigma))) / (sigma*sigma*sigma);
            rec_solv += (4.0/3.0) * PI * pow(radii[a], 3) * exp((-d2/(2*sigma*sigma))) / (sigma*sigma*sigma);
