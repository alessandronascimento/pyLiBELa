...
public:
  ...
  //! C-style linear array version of grids
  double *r_elec_grid{};
  double *r_pbsa_grid{};
  double *r_vdwA_grid{};
  double *r_vdwB_grid{};
  double *r_solv_gauss{};
  double *r_rec_solv_gauss{};
  double *r_delphi_grid{};
  double *r_hb_donor_grid{};
  double *r_hb_acceptor_grid{};
  ...
  /*!
   * \brief Grid This function initializes the class. It passes a copy of the
   * PARSER class to the Grid class and defines the grid spacing \param _Input
   * Pointer to the PARSER class. \param _Writer Pointer to the WRITER class.
   */
  Grid(PARSER *_Input, WRITER *_Writer);
  /*!
   * \brief Grid Initializer of the Grid class used through the code. This
   * method copies a pointer to the class PARSER to this class, defines the grid
   * spacing and calls the appropriate methods to start grid computation. Also,
   * if the keyword "write_grids" is set to "yes" in the PARSER, it calls the
   * method to write the grids to a file. \param _Input Pointer to the PARSER
   * class. \param _Writer Pointer to the WRITER class. \param Rec POINTER to a
   * Mol2 class with Receptor information. \param com C++ vector with the
   * coordinates of the center of mass of the reference ligand. It is used to
   * define the center of the computation box.
   */
  ...
  double distance(double x1, double x2, double y1, double y2, double z1,
                  double z2);

  double distance_squared(double x1, double x2, double y1, double y2, double z1,
                          double z2);
  ...
  /**
   * @brief write_grids_to_file This function writes a computed potential grid
   * to a file
   */
  void write_grids_to_file(void);
  /**
   * @brief load_grids_from_file This function loads a pre-computed Grid file,
   * computed with LiBELa or McGrid
   */
  void load_grids_from_file(void);
  /**
   * @brief load_Ambergrids_from_file This function loads a PBSA potential file
   * as computed by pbsa program in AmberTools
   */
  void load_Ambergrids_from_file(void);

  /**
   * @brief load_gzAmbergrids_from_file This function loads a gzipped PBSA
   * potential file as computed by pbsa program in AmberTools.
   */
  void load_gzAmbergrids_from_file(void);

  /**
   * @brief load_Delphi_Grid_from_file This fuction loads a Delphi potential
   * file in binary BIOSYM format file
   */
  ...
  /**
   * @brief compute_grid_hardcore_omp This function computes hardcore (Amber FF)
   * potential using multicore implementation with OpenMP.
