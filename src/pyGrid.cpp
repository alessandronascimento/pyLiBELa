...
double Grid::distance(double x1, double x2, double y1, double y2, double z1,
                      double z2) {
...
double Grid::distance_squared(double x1, double x2, double y1, double y2,
                              double z1, double z2) {
  return ((((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1)) +
           ((z2 - z1) * (z2 - z1))));
}

double Grid::angle(double x1, double y1, double z1, double x2, double y2,
...
Grid::Grid(PARSER *_Input, WRITER *_Writer, Mol2 *Rec, vector<double> com) {
  ...
  if (Input->pbsa_grid != "" and Input->use_pbsa) {
    ...
    if (Input->delphi_cube_grid.substr(Input->delphi_cube_grid.size() - 3, 3) ==
        ".gz") {
...
void Grid::compute_grid_softcore(Mol2 *Rec) {
  if (Input->scoring_function < 2) {
    vector<double> elec_t1(npointsz), vdwA_t1(npointsz), vdwB_t1(npointsz),
        solv_t1(npointsz), rec_solv_t1(npointsz);
      ...
      for (int b = 0; b < this->npointsy; b++) {
        ...
        for (int c = 0; c < this->npointsz; c++) {
          ...
          for (int i = 0; i < Rec->N; i++) {
            d = this->distance(x, Rec->xyz[i][0], y, Rec->xyz[i][1], z,
                               Rec->xyz[i][2]);
            d2 = d * d;
            d3 = d * d * d;
            d6 = d2 * d2 * d2;
            ...
            if (Input->dielectric_model == "constant") {
              ...
              solv += ((Input->solvation_alpha * Rec->charges[i] *
                        Rec->charges[i]) +
                       Input->solvation_beta) *
                      exp((-denom / (2 * Input->sigma * Input->sigma))) /
                      (Input->sigma * Input->sigma * Input->sigma);
              rec_solv += (4.0 / 3.0) * PI * pow(Rec->radii[i], 3) *
                          exp((-denom / (2 * Input->sigma * Input->sigma))) /
                          (Input->sigma * Input->sigma * Input->sigma);
            }
            ...
            vdwA += (4096.0 * Rec->epsilons_sqrt[i] * pow(Rec->radii[i], 6)) /
                    (denom * denom);
            vdwB +=
                (128.0 * Rec->epsilons_sqrt[i] * pow(Rec->radii[i], 3)) / denom;
          }
          elec_t1[c] = (elec);
          vdwA_t1[c] = (vdwA);
          vdwB_t1[c] = (vdwB);
    ...
    for (int i = 0; i < Rec->N; i++) {
      this->rec_si +=
          (Input->solvation_alpha * Rec->charges[i] * Rec->charges[i]) +
          Input->solvation_beta;
    }
  } else {
      ...
      for (int a = 0; a < this->npointsx; a++) {
        ...
        for (int b = 0; b < this->npointsy; b++) {
          ...
          for (int c = 0; c < this->npointsz; c++) {
            ...
            for (int i = 0; i < Rec->N; i++) {
              double d = this->distance(x, Rec->xyz[i][0], y, Rec->xyz[i][1], z,
                                        Rec->xyz[i][2]);
              double d2 = d * d;
              double d3 = d * d * d;
              double d6 = d2 * d2 * d2;
              ...
              vdwA += (4096.0 * Rec->epsilons_sqrt[i] * pow(Rec->radii[i], 6)) /
                      (denom * denom);
              vdwB += (128.0 * Rec->epsilons_sqrt[i] * pow(Rec->radii[i], 3)) /
                      denom;
            }
...
void Grid::compute_grid_softcore_HB_omp(Mol2 *Rec) {
  if (Input->scoring_function < 2) {
    vector<double> elec_t1(npointsz), vdwA_t1(npointsz), vdwB_t1(npointsz),
        solv_t1(npointsz), rec_solv_t1(npointsz), hb_donor_t1(npointsz),
        hb_acceptor_t1(npointsz);
    vector<vector<double>> elec_t2, vdwA_t2, vdwB_t2, solv_t2, rec_solv_t2,
        hb_donor_t2, hb_acceptor_t2;

    // Constants for HB calculation using a 10-12 potential.
    // Values taken from AutoDock paper. They were computed to result in -5
    // kcal/mol in energy for a good HB
      ...
      for (int a = 0; a < this->npointsx; a++) {
        ...
        for (int b = 0; b < this->npointsy; b++) {
          ...
          for (int c = 0; c < this->npointsz; c++) {
            ...
            for (int i = 0; i < Rec->N; i++) {
              double d2 = this->distance_squared(
                  x, Rec->xyz[i][0], y, Rec->xyz[i][1], z, Rec->xyz[i][2]);
              double d6 = d2 * d2 * d2;
            ...
            for (unsigned i = 0; i < Rec->HBdonors.size(); i++) {
              double d2 = distance_squared(Rec->xyz[Rec->HBdonors[i][1]][0], x,
                                           Rec->xyz[Rec->HBdonors[i][1]][1], y,
                                           Rec->xyz[Rec->HBdonors[i][1]][2], z);
              double d10 = d2 * d2 * d2 * d2 * d2;
              double ang = angle(Rec->xyz[Rec->HBdonors[i][0]][0],
                                 Rec->xyz[Rec->HBdonors[i][0]][1],
                                 Rec->xyz[Rec->HBdonors[i][0]][2],
                                 Rec->xyz[Rec->HBdonors[i][1]][0],
                                 Rec->xyz[Rec->HBdonors[i][1]][1],
                                 Rec->xyz[Rec->HBdonors[i][1]][2], x, y, z);
              double angle_term = cos(ang * PI / 180.0) *
                                  cos(ang * PI / 180.0) *
            ...
            for (unsigned i = 0; i < Rec->HBacceptors.size(); i++) {
              double d2 = distance_squared(Rec->xyz[Rec->HBacceptors[i]][0], x,
                                           Rec->xyz[Rec->HBacceptors[i]][1], y,
                                           Rec->xyz[Rec->HBacceptors[i]][2], z);
              double d10 = d2 * d2 * d2 * d2 * d2;
...
void Grid::load_grids_from_file() {
  ...
  vector<double> elec_t1(npointsz), vdwA_t1(npointsz), vdwB_t1(npointsz),
      solv_t1(npointsz), rec_solv_t1(npointsz), pbsa_t1(npointsz),
      delphi_t1(npointsz), hb_donor_t1(npointsz), hb_acceptor_t1(npointsz);
  vector<vector<double>> elec_t2, vdwA_t2, vdwB_t2, solv_t2, rec_solv_t2,
      pbsa_t2, delphi_t2, hb_donor_t2, hb_acceptor_t2;
...
Grid::~Grid() {
  ...
  delete[] r_vdwB_grid;
  delete[] r_solv_gauss;
  delete[] r_rec_solv_gauss;
  delete[] r_delphi_grid;
  delete[] r_hb_donor_grid;
  delete[] r_hb_acceptor_grid;
}
  ...
  for (int a = 0; a < this->npointsx; a++) {
    ...
    for (int b = 0; b < this->npointsy; b++) {
      ...
      for (int c = 0; c < this->npointsz; c++) {
        ...
        for (int i = 0; i < Rec->N; i++) {
          d2 = this->distance_squared(x, Rec->xyz[i][0], y, Rec->xyz[i][1], z,
                                      Rec->xyz[i][2]);
          d6 = d2 * d2 * d2;
          if (Input->dielectric_model == "constant") {
            d = sqrt(d2);
          ...
          vdwA +=
              Rec->epsilons_sqrt[i] * 64.0 * pow(Rec->radii[i], 6) / (d6 * d6);
          vdwB +=
              sqrt2 * Rec->epsilons_sqrt[i] * 8.0 * pow(Rec->radii[i], 3) / d6;
          ...
          solv +=
              ((Input->solvation_alpha * Rec->charges[i] * Rec->charges[i]) +
               Input->solvation_beta) *
              exp((-(deff) / (2 * Input->sigma * Input->sigma))) /
              (Input->sigma * Input->sigma * Input->sigma);
          rec_solv += (4.0 / 3.0) * PI * pow(Rec->radii[i], 3) *
                      exp((-(deff) / (2 * Input->sigma * Input->sigma))) /
                      (Input->sigma * Input->sigma * Input->sigma);
...
void Grid::load_Ambergrids_from_file() {
  ...
  if (Input->pbsa_grid.substr(Input->pbsa_grid.size() - 3, 3) == ".gz") {
    ...
    fscanf(pbsa_map, "%d %d %d", &this->npointsx, &this->npointsy,
           &this->npointsz);
      ...
      for (int b = 0; b < this->npointsy; b++) {
        ...
        for (int c = 0; c < this->npointsz; c++) {
          ...
          for (int i = 0; i < Rec->N; i++) {
            ...
            vdwA += Rec->epsilons_sqrt[i] * 64.0 * pow(Rec->radii[i], 6) /
                    (d6 * d6);
            vdwB += sqrt2 * Rec->epsilons_sqrt[i] * 8.0 *
                    pow(Rec->radii[i], 3) / d6;
  ...
  for (int z = 0; z < intz + 1; z++) {
    for (int y = 0; y < inty + 1; y++) {
      for (int x = 0; x < intx + 1; x++) {
        ...
        this->delphi_grid[x][y][z] =
            (0.593f * phi); // potential is given in KT/e
      }
    }
  }
...
void Grid::load_phimap_from_file(int gsize) {
  ...
  for (int nz = 0; nz < gsize; nz++) {
    for (int ny = 0; ny < gsize; ny++) {
      for (int nx = 0; nx < gsize; nx++) {
        ...
        this->delphi_grid[nx][ny][nz] =
            0.593f * kt_phi; // converting kt units to kcal/mol
      }
    }
  }
  ...
  if (igrid != gsize) {
    printf("Mismatch between GSIZE given and found in phimap file. Please "
           "check.\n");
    exit(1);
  }
  ...
  sprintf(info, "DelPhi Grid file %s read!", Input->delphi_grid.c_str());
  this->print_info(info);
  sprintf(info, "DelPhi Grid Scale / Spacing: %7.4f %7.4f", scale,
          this->grid_spacing);
  this->print_info(info);
  sprintf(info, "DelPhi Grid GSIZE: %5d. Mid XYZ: %7.4f %7.4f %7.4f", gsize,
          oldmid_x, oldmid_y, oldmid_z);
  this->print_info(info);
  sprintf(info,
          "Grid Dimensions XYZ: [%7.4f <-> %7.4f] [%7.4f <-> %7.4f] [%7.4f <-> "
          "%7.4f]",
          this->xbegin, this->xend, this->ybegin, this->yend, this->zbegin,
          this->zend);
  this->print_info(info);
...
void Grid::load_delphi_cube() {
  ...
  fscanf(phimap, "%f %d %f %f %f", &scale, &igrid, &centerx, &centery,
         &centerz);
  ...
  for (int nx = 0; nx < igrid; nx++) {
    for (int ny = 0; ny < igrid; ny++) {
      for (int nz = 0; nz < igrid; nz++) {
        fscanf(phimap, "%f", &phi);
        this->delphi_grid[nx][ny][nz] =
            double(0.593f * phi); // converting kt units to kcal/mol
      }
    }
  }
  ...
  sprintf(info, "DelPhi CUBE Grid file %s read!",
          Input->delphi_cube_grid.c_str());
  this->print_info(info);
  sprintf(info, "DelPhi CUBE Grid Spacing: %7.4f", this->grid_spacing);
  this->print_info(info);
  sprintf(info, "DelPhi CUBE Grid GSIZE: %5d. Center: %7.4f %7.4f %7.4f", igrid,
...
void Grid::load_delphi_gzcube() {
  ...
  if (phimap == NULL) {
    printf("Could not open DelPhi Grid file %s. Please check",
           Input->delphi_cube_grid.c_str());
    exit(1);
  }
...
void Grid::compute_grid_hardcore_Gaussian(Mol2 *Rec) {
  ...
  double elec, d, d2, d6, x, y, z, vdwA, vdwB, solv, rec_solv, deff,
      LJ_gauss_weight, Coulomb_gauss_weight;
  double sqrt2 = sqrt(2.0);
    ...
    for (int b = 0; b < this->npointsy; b++) {
      ...
      for (int c = 0; c < this->npointsz; c++) {
        ...
        for (int i = 0; i < Rec->N; i++) {
          ...
          dw > 0.
              ? LJ_gauss_weight = exp(-((d - (Rec->radii[i] + Rec->radii[i])) *
                                        (d - (Rec->radii[i] + Rec->radii[i]))) /
                                      (2.0 * Input->LJ_sigma * Input->LJ_sigma))
              : -1.0;

          Coulomb_gauss_weight =
              exp(-((d - (Rec->radii[i] + Rec->radii[i])) *
                    (d - (Rec->radii[i] + Rec->radii[i]))) /
                  (2.0 * Input->coulomb_sigma * Input->coulomb_sigma));
          ...
          if (Input->dielectric_model == "constant") {
            ...
            if (Input->use_GW_Coulomb) {
              elec +=
                  (332.0 * (Rec->charges[i] / (4 * d2))) * Coulomb_gauss_weight;
            } else {
              elec += (332.0 * (Rec->charges[i] / (4 * d2)));
            }
          } else { // Input->dielectric_model = "r"
          ...
          if (Input->use_GW_LJ12) {
            LJ_gauss_weight > 0
                ? vdwA += (Rec->epsilons_sqrt[i] * 64.0 *
                           pow(Rec->radii[i], 6) / (d6 * d6)) *LJ_gauss_weight
                : vdwA += MAX_ALJ;
          } else {
            vdwA += (Rec->epsilons_sqrt[i] * 64.0 * pow(Rec->radii[i], 6) /
                     (d6 * d6));
          ...
          if (Input->use_GW_LJ6) {
            LJ_gauss_weight > 0
                ? vdwB += (sqrt2 * Rec->epsilons_sqrt[i] * 8.0 *
                           pow(Rec->radii[i], 3) / d6) *LJ_gauss_weight
                : vdwB += MAX_BLJ;
          } else {
            vdwB += (sqrt2 * Rec->epsilons_sqrt[i] * 8.0 *
                     pow(Rec->radii[i], 3) / d6);
      ...
      for (int b = 0; b < this->npointsy; b++) {
        ...
        for (int c = 0; c < this->npointsz; c++) {
          ...
          for (int i = 0; i < Rec->N; i++) {
            ...
            double dw = d - (1.2 * Rec->radii[i]);
            dw > 0. ? LJ_gauss_weight =
                          exp(-((d - (Rec->radii[i] + Rec->radii[i])) *
                                (d - (Rec->radii[i] + Rec->radii[i]))) /
                              (2.0 * Input->LJ_sigma * Input->LJ_sigma))
                    : -1.0;
            double Coulomb_gauss_weight =
                exp(-((d - (Rec->radii[i] + Rec->radii[i])) *
                      (d - (Rec->radii[i] + Rec->radii[i]))) /
                    (2.0 * Input->coulomb_sigma * Input->coulomb_sigma));
          ...
          for (unsigned i = 0; i < Rec->HBdonors.size(); i++) {
            double d = distance(Rec->xyz[Rec->HBdonors[i][1]][0], x,
                                Rec->xyz[Rec->HBdonors[i][1]][1], y,
                                Rec->xyz[Rec->HBdonors[i][1]][2], z);
            ...
            double angle_term = cos(ang * PI / 180.0) * cos(ang * PI / 180.0) *
                                cos(ang * PI / 180.0) * cos(ang * PI / 180.0);
            hb_donor +=
                ((HB_C12 / (d10 * d * d)) - (HB_C10 / d10)) * angle_term;
          }

          for (unsigned i = 0; i < Rec->HBacceptors.size(); i++) {
            double d = distance(Rec->xyz[Rec->HBacceptors[i]][0], x,
                                Rec->xyz[Rec->HBacceptors[i]][1], y,
                                Rec->xyz[Rec->HBacceptors[i]][2], z);
            double d10 = d * d * d * d * d * d * d * d * d * d;
...
BOOST_PYTHON_MODULE(pyGrid) {
  ...
  class_<Grid>("Grid", init<PARSER *, WRITER *, Mol2 *, vector<double>>())
      ...
      .def("compute_grid_hardcore_Gaussian",
           &Grid::compute_grid_hardcore_Gaussian)
      ...
      .def("compute_grid_hardcore_omp_HB_Gaussian",
           &Grid::compute_grid_hardcore_omp_HB_Gaussian)
